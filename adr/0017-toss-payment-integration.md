...
이 경합과 겉보기에 비슷하지만 성격이 다른 경합이 하나 더 있다 - 같은 주문에 걸린 **서로
다른 결제 시도 두 개**가 동시에 confirm 되는 경우다. 이건 취소가 끼지 않아도 일어나고,
Toss 가 두 시도를 각각 실제로 승인해 버려 이중 결제(실제 과금)로 이어진다는 점에서 더
위험하다. [0019](0019-concurrent-confirm-claim.md)가 이 경합만 따로 떼어 닫았다 - 지금
이 절에서 방치하기로 한 취소·confirm 경합은 그 ADR 도 다루지 않으므로 여전히 남아 있다.
...
- 결제 완료·취소 사이의 경합 창을 완전히 막지 않는다(위 근거 참고). 발생하면 로그로만
  드러나고 정산은 수동이다. ([0019](0019-concurrent-confirm-claim.md)가 닫은 것은
  이것과 다른, 동시 confirm 두 개 사이의 경합이다.)
- 가상계좌, 부분 환불, 웹훅 기반 비동기 상태 반영이 전부 빠졌다. (결제 완료 주문의
  취소·환불 자체는 이후 [0018](0018-payment-refund-via-order-cancel.md)이 열었다 -
  여기서는 막아 두기만 했다.)
...
`PaymentStatus` 에 상태가 더 늘어난다 - 부분 환불은 [0018](0018-payment-refund-via-order-cancel.md)
이후에도 여전히 범위 밖이다.

결제 완료 주문의 환불 자동화는 [0018](0018-payment-refund-via-order-cancel.md)이 했다 -
`CancelOrderService` 가 `PAID` 상태까지 확장해 `PaymentGateway` 의 취소 API 를 부른다.

같은 주문의 서로 다른 결제 시도가 동시에 confirm 되어 이중 결제로 이어지는 경합은
[0019](0019-concurrent-confirm-claim.md)가 막았다. 다만 그 ADR 은 의도적으로 범위를
좁혀, 바로 위에서 방치하기로 한 취소·confirm 경합(취소가 `cancelIfPlaced` 로 주문을
`CANCELLED` 로 옮기는 사이 confirm 이 Toss 승인을 받아 버리는 경우)은 여전히 막지 않는다.
[0019](0019-concurrent-confirm-claim.md)가 도입한 `orders.active_payment_id` 선점을
`cancelIfPlaced` 도 함께 확인하도록(선점 중이면 취소를 거절하거나 대기시키도록) 확장하면
이 경합도 닫을 수 있다 - 아직 만들지 않았다.
