...
- 한 주문에 `DONE` 결제가 둘 이상 존재하는(동시 `confirm` 경합의 결과물) 이상 상태를
  ...
  환불되고 나머지는 조용히 남는다. ([0019](0019-concurrent-confirm-claim.md)가 이
  경합이 새로 생기는 경로 자체를 막았으므로, 이 방어 코드는 그 이전 데이터나 아직
  발견하지 못한 다른 경로를 위한 것으로 남는다.)
...
한 주문에 `DONE` 결제가 둘 이상 생기는 경합은 [0019](0019-concurrent-confirm-claim.md)가
닫았다 - `findDoneByOrderId`가 여럿 중 하나만 돌려주는 방어 코드는 그대로 남지만, 새로
그런 상태가 생길 경로는 더 이상 없다. `cancelIfPaid` 도 `cancelIfPlaced`/`markPaidIfPlaced`
와 같은 단일 행 CAS 라 이번에 새로 위험해지지 않는다.
