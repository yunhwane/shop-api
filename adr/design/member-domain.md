...
인증 요청과 로그인에 호출 제한이 걸린다([0009](../0009-abuse-rate-limiting.md)).
인증 요청은 IP 와 이메일 두 열쇠로, 로그인은 IP 로만 그리고 실패만 센다 —
아이디 단위로 막으면 공격자가 남의 계정을 잠글 수 있다.

기한이 지난 인증과 리프레시 토큰은 주기적으로 지운다([0010](../0010-expired-data-retention.md)).
기준은 만료 하나이고 소비 여부는 보지 않는다. 소비된 행을 일찍 지우면 재사용 탐지가
무너지기 때문이다.

**두 결정이 맞물려 있다.** [0008](../0008-login-token-strategy.md) 에서 만료를 소비보다
먼저 검사하도록 고쳤기 때문에 만료된 행을 지워도 안전하다. 그 순서를 되돌리면
이 배치가 유출 탐지에 구멍을 낸다.
